...
class Problem:
    ...
    def successor(self, state):
        ...
        abstract # extent so it complies

        #return list(actions, next_states)
    ...
    def value(self):
        ...
        How creates values for the states?
        """
...
def tree_search(problem, fringe):
    ...
    fringe.append(Node(problem.initial))  # create Node (state, parent, action, )
    exploredNodes = 0
    startTime = time.perf_counter()
    while fringe : #and (time.perf_counter()-startTime < 180)
        node = fringe.pop()
        exploredNodes += 1
...
def graph_search(problem, fringe):
    ...
    while fringe : # and (time.perf_counter()-startTime < 180)
        ...
        if node.state not in closed:
            ...
            fringe.extend(node.expand(problem, closed)) # closed becomes visited
    return None,exploredNodes, len(fringe)
